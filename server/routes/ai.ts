...
router.post("/enhance-prompt", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const messages: GroqMessage[] = [
      {
        role: "system",
        content: `אתה עוזר שמשפר שאלות של משתמשים למערכת HomeBrain — מערכת שעונה על שאלות לגבי מכשירי בית על סמך מסמכים שהועלו (מדריכי הפעלה, אחריות, הוראות התקנה).

התפקיד שלך: לקחת את השאלה הגולמית של המשתמש ולהפוך אותה לשאלה ברורה, ספציפית ומדויקת יותר, כך שמערכת ה-RAG תוכל למצוא את התשובה הטובה ביותר.

כללים:
1. שמור על הכוונה המקורית של המשתמש
2. הוסף פרטים רלוונטיים אם הנושא ידוע (שם המכשיר/מוצר)
3. הפוך שאלות מעורפלות לספציפיות
4. שמור על עברית טבעית
5. החזר רק את השאלה המשופרת, בלי הסברים
6. אם השאלה כבר טובה — החזר אותה כמו שהיא
7. אל תוסיף מידע שלא משתמע מהשאלה המקורית`,
      },
      {
        role: "user",
